...
def shallownn(x):
    ...
    norm11 = tf.layers.batch_normalization(
        inputs=fc11,
        name='norm11',
        axis=-1,
        momentum=0.99,
...
def deepnn(x):
    ...
    def firstlayer(img):
        ...
        normconv12 = tf.layers.batch_normalization(
            inputs=conv12,
            name='normconv12',
            axis=-1,
            momentum=0.99,
    ...
    def seclayer(img):
        ...
        normconv21 = tf.layers.batch_normalization(
            inputs=conv21,
            name='normconv21',
            axis=-1,
            momentum=0.99,
        ...
        # add batch normaisation
        normconv22 = tf.layers.batch_normalization(
            inputs=conv22,
            name='normconv22',
            axis=-1,
            momentum=0.99,
    ...
    # add batch normalisation layer
    normfc1 = tf.layers.batch_normalization(
        inputs=fc1,
        name='normfc1',
        axis=-1,
        momentum=0.99,
...
def runNet(netType, augment):
    ...
    maxSteps = int(dataSize/16)
    learn = tf.train.exponential_decay(learningRate, maxSteps, 300, 0.6)
    # Define your AdamOptimiser, using FLAGS.learning_rate to minimise the loss function
    optimiser = tf.train.AdamOptimizer(learn, beta1=0.9, beta2=0.999, epsilon=1e-08, ).minimize(
        regularized_loss)
